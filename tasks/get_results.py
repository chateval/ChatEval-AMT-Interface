...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description="Get results from MTurk task")
    parser.add_argument("--credentials", required=True, type=str, help="Path to the file containing MTurk credentials")
    parser.add_argument("--sandbox", required=True, type=int, help="Whether to publish on sandbox or the marketplace")
    parser.add_argument("--ids", required=True, type=str, help="Path to where to the relevant HITIDs are stored.  This file is produced by create_tasks.py")
    parser.add_argument("--output", "-o", required=True, type=str, help="Path to the desired output file, should be a .csv")
    args = parser.parse_args()

    mturk = connectToMTurk(args.credentials, args.sandbox)
    HITIDsfile = args.ids
