...
def main(credsfile, inputfile, id_file_name, link_file_name, tracking_file_name):
  ...
  for i in range(20,60,5):   
    ...
    new_hit = mturk.create_hit(
        Title = 'Evaluate dialog agents\' conversation responses',
        Description = 'For 5 conversations judge whether different programs responses are appropriate',
...
def response_eval(prompts_file,responses_path):
  prompts = []
  bad_indices = [7, 18, 56, 63, 74, 81, 169, 180]
  ...
  for path in paths:
    ...
    print (all_responses[bot_name][0])
  for index,prompt in enumerate(prompts):
    turns = []
    for turn_num, turn in enumerate(prompt.split("</s>")):
      if turn_num % 2==0:
        turns.append("A: {}".format(turn))
      else:
        turns.append("B: {}".format(turn))
    all_data[index] = {"prompt":turns, "models": [], "qc": -1}
    ...
    all_data[index]["models"].append({"response": "Enter {} for each button, it is quality control".format(qc), "bot":"quality_control"})
